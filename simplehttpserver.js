...
var argv = require('yargs')
    ...
    .option('r', {
        alias: 'proxy',
        describe: 'forward request to other server',
        type: 'string'
    })
...
// reverse proxy
if (proxySettings) {
    var proxies = proxySettings.split(',');
    for (const proxySetting of proxies) {
        const parts = proxySetting.split('=');
        if (parts.length == 2) {
            var vpath = parts[0].trim();
            var target = parts[1].trim();

            var apiProxy = createProxyMiddleware(vpath, {
                target: target,
                logLevel: 'debug',
                changeOrigin: false
